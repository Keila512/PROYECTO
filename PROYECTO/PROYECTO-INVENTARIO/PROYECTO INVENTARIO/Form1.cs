...
namespace PROYECTO_INVENTARIO
{
    public partial class Form1 : Form
    {
        ...
        {
            DataGridViewRow articulo = null;

            foreach (DataGridViewRow row in dgvBoard.Rows)
            {
                if (row.Cells["Codigo"].Value != null && row.Cells["Codigo"].Value.ToString() == code)
                {
                    articulo = row;
                    dgvBoard.Rows.Remove(row);
                    break;
                }
            }
            if (articulo != null)
            {
                articulo.Cells["stock"].Value = int.Parse(articulo.Cells["entry"].Value.ToString()) - int.Parse(articulo.Cells["exit"].Value.ToString());
            }
        ...
        private void fill_DGV(ProductDetails product)
        {
            ...
            try
            {
                ...
                row.Cells[1].Value = product.Entry_date;
